import { useRef, useState, useEffect, KeyboardEvent, PointerEvent } from "react";
import { Button } from "@/components/ui/button";
import { FileUp, FolderUp, Paperclip, Send, X } from "lucide-react";
      ...
      <div className="relative bg-transparent">
        ...
        <div className="flex items-center gap-2 max-w-[780px] mx-auto px-3 py-2.5">
          ...
          <div className="flex items-center gap-1.5 shrink-0 self-end pb-[8px]">
            ...
            >
              <Paperclip className={`h-[18px] w-[18px] transition-transform duration-200 ${attachmentsOpen ? "rotate-45" : ""}`} />
            </Button>

            {/* Expanded: File + Folder buttons */}
            <div className={`flex items-center gap-1.5 overflow-hidden transition-all duration-200 ${attachmentsOpen ? "max-w-24 opacity-100" : "max-w-0 opacity-0"}`}>
              {/* Single file */}
              <Button
                type="button"
                variant="ghost"
          ...
          <button
            ...
            className={[
              ...
              value.trim()
                ? "bg-primary text-primary-foreground shadow-[0_0_18px_hsl(var(--primary)/0.45)] hover:shadow-[0_0_26px_hsl(var(--primary)/0.65)] hover:scale-105 active:scale-95 cursor-pointer"
